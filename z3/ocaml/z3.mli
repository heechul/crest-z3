...
(**
   ...
   - model: a model for the constraints asserted into the logical context.
*)
(**
    {!lbool}  
...
(**
   ...
   - {b See also}: {!Z3.get_decl_num_parameters}
   - {b See also}: {!Z3.get_decl_parameter_kind}
...
(**
   ...
   - EXCEPTION:     Internal Z3 exception. Additional details can be retrieved using get_error_msg_ex.
*)
(**
    {!goal_prec}  
...
(**
       ...
       \deprecated Use {!Z3.mk_context_rc}
    *)
...
(**
       ...
       - {b Remarks}: Reference counting must be used to manage parameter sets, even when the context was
       created using {!Z3.mk_context} instead of {!Z3.mk_context_rc}.
    *)
...
(**
       ...
       - {b See also}: {!Z3.get_finite_domain_sort_size.}
    *)
...
(**
       ...
       See the documentation for {!Z3.mk_forall} for an explanation of the parameters.
...
(**
       Summary: Convert a sort into ast. 
        - {b Remarks}: [Z3.sort_to_ast c s] can be replaced by [(s :> Z3.ast)]. 
    *)
...
(**
       Summary: Convert a func_decl into ast. 
        - {b Remarks}: [Z3.func_decl_to_ast c f]  can be replaced by [(f :> Z3.ast)]. 
    *)
...
(**
       Summary: Convert a app into ast. 
        - {b Remarks}: [Z3.app_to_ast c a] can be replaced by [(a :> Z3.ast)]. 
    *)
...
(**
        Summary: Return a unique identifier for t.
         Implicitly used by [Pervasives.compare] for values of type [ast], [app], [sort], [func_decl], and [pattern]. 
    *)
...
(**
       Summary: Return a hash code for the given AST.
        Implicitly used by [Hashtbl.hash] for values of type [ast], [app], [sort], [func_decl], and [pattern]. 
    *)
...
(**
       Summary: Convert a pattern into ast. 
        - {b Remarks}: [Z3.pattern_to_ast c p]  can be replaced by [(p :> Z3.ast)]. 
    *)
...
(**
       ...
       - {b See also}: {!Z3.model_get_const_decl}
    *)
...
(**
       ...
       - {b See also}: {!Z3.model_get_num_funcs}
    *)
...
(**
       ...
       - {b See also}: {!Z3.model_get_sort}
       - {b See also}: {!Z3.model_get_sort_universe}
    *)
...
(**
       ...
       - {b See also}: {!Z3.model_get_num_sorts}
       - {b See also}: {!Z3.model_get_sort_universe}
    *)
...
(**
       ...
       - {b See also}: {!Z3.get_as_array_func_decl}
    *)
...
(**
       ...
       - {b See also}: {!Z3.func_interp_get_num_entries}
    *)
...
(**
       ...
       - {b See also}: {!Z3.func_interp_get_entry}
    *)
...
(**
    ...
    *)
(**
       Summary: Return the error code for the last API call.
       
       A call to a Z3 function may return a non OK error code,
       when it is not used correctly.

       - {b See also}: {!Z3.set_error_handler}
    *)
...
(**
       ...
       \deprecated Use {!Z3.get_error_msg_ex} instead.
    *)
...
(**
        ...
        - L_TRUE if the query is satisfiable. Obtain the answer by calling {!Z3.fixedpoint_get_answer}.
...
(**
       ...
       The complete list of tactics may be obtained using the procedures {!Z3.get_num_tactics} and {!Z3.get_tactic_name}.
...
(**
       ...
       The complete list of probes may be obtained using the procedures {!Z3.get_num_probes} and {!Z3.get_probe_name}.
       It may also be obtained using the command {e (help-tactics) } in the SMT 2.0 front-end.
...
(**
       ...
       - {b Remarks}: For probes, "true" is any value different from 0.
    *)
...
(**
       ...
       It behaves like {!Z3.mk_solver} if the logic is unknown or unsupported.
...
(**
       ...
       The solver supports the commands {!Z3.solver_push} and {!Z3.solver_pop}, but it
       will always solve each {!Z3.solver_check} from scratch.
    *)
...
(**
       Summary: Retrieve the unsat core for the last {!Z3.solver_check_assumptions}
...
(**
       ...
       the commands {!Z3.solver_check} and {!Z3.solver_check_assumptions}
    *)
...
(**
       ...
       {!Z3.assert_cnstr}, {!Z3.check_assumptions}, etc.
    *)
...
(**
       ...
       Use {!Z3.stats_to_string} when using the new solver API.
    *)
