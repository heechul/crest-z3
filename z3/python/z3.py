...
class FuncDeclRef(AstRef):
	...
	args = _get_args(args)
        ...
        _args = (Ast * num)()
        for i in range(num):
            _args[i] = self.domain(i).cast(args[i]).as_ast()
...
class Fixedpoint(Z3PPObject):
	...
	return ExprRef(r, self.ctx)

    def register_relation(self, f):
...
class Tactic:
    ...
    def help(self):
        """Return a string containing a description of the available options for the `self` tactic."""
